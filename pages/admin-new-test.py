...
def generate_profile_content(comm, motiv):
    ...
    integrated_profile_lines = []
    integrated_profile_lines.append(f"**Profile:** {title}")
    if synergy:
        integrated_profile_lines.append(f"\n**How this profile shows up (the blend):**\n{synergy}")

    if comm_keypoints:
        integrated_profile_lines.append("\n**Communication wiring (what you’ll reliably see):**")
        for b in comm_keypoints:
            if b:
                integrated_profile_lines.append(f"- {b}")

    if motiv_keypoints:
        integrated_profile_lines.append("\n**Motivation wiring (what fuels their best work):**")
        for b in motiv_keypoints:
            if b:
                integrated_profile_lines.append(f"- {b}")

    if thriving_snippet:
        integrated_profile_lines.append("\n**Leadership strengths when they’re at their best:**")
        integrated_profile_lines.append(thriving_snippet)

    if struggling_snippet:
        integrated_profile_lines.append("\n**Likely drift under stress (early warning signs):**")
        integrated_profile_lines.append(struggling_snippet)

    integrated_profile_lines.append("\n**What helps them lead well (fast supervisor moves):**")
    if supervisor_moves_comm:
        integrated_profile_lines.append("- **Communication support:**")
        for b in supervisor_moves_comm:
            integrated_profile_lines.append(f"  - {b}")
    if supervisor_moves_motiv:
        integrated_profile_lines.append("- **Motivation support:**")
        for b in supervisor_moves_motiv:
            integrated_profile_lines.append(f"  - {b}")
