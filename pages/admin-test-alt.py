...
st.caption(
    "A structured, phase-based coaching framework. "
    "Each phase is independent and collapsible to prevent overload and system crashes."
)

def build_coaching_matrix_safe(p_comm, s_comm, p_mot, s_mot, phase):
    phase_cards = {
        1: {
            "title": "Phase 1 – Safety & Consistency",
            "aim": "Build reliable baseline behavior, documentation, and supervision habits.",
            "supervisor_role": "Coach closely, reduce ambiguity, reinforce minimum safe standards.",
            "common_pitfall": "Assuming competence before consistency exists.",
        },
        2: {
            "title": "Phase 2 – Judgment & Anticipation",
            "aim": "Develop risk assessment, decision-making, and pattern recognition.",
            "supervisor_role": "Require recommendations with rationale; stop rescuing.",
            "common_pitfall": "Over-escalation or rigid rule-following to avoid risk.",
        },
        3: {
            "title": "Phase 3 – Ownership & Systems Thinking",
            "aim": "Shift from managing moments to improving systems and delegating leadership.",
            "supervisor_role": "Delegate outcomes, review trends, coach other leaders through them.",
            "common_pitfall": "Holding authority too tightly or avoiding hard calls.",
        },
    }

    moves = [
        {
            "title": "1) Name the Standard",
            "why": "Clarity reduces anxiety and prevents rework.",
            "how": "State the expectation clearly before discussing feelings or context.",
            "scripts": ["“Here’s the standard we’re working toward.”"],
            "avoid": "Hinting or softening until the standard becomes unclear.",
        },
        {
            "title": "2) Transfer Ownership",
            "why": "Growth requires decision-making responsibility.",
            "how": "Ask for a recommendation with rationale and risk awareness.",
            "scripts": ["“What do you recommend, and why?”"],
            "avoid": "Solving it for them too early.",
        },
        {
            "title": "3) Align Motivation",
            "why": "People sustain effort when work connects to what drives them.",
            "how": "Link the task to purpose, growth, connection, or achievement.",
            "scripts": ["“This matters because it protects youth and the team.”"],
            "avoid": "Reducing everything to compliance.",
        },
        {
            "title": "4) Stress-Test the Plan",
            "why": "Anticipation prevents crisis escalation.",
            "how": "Ask what could go wrong and how they would respond.",
        ...
        },
        {
            ...
            "how": "Name exactly when supervision must be involved.",
            "scripts": ["“Call immediately if X happens.”"],
            "avoid": "Waiting too long to escalate.",
        },
        {
            "title": "6) Close the Learning Loop",
            "why": "Reflection turns experience into growth.",
            "how": "Debrief outcomes, not just compliance.",
            "scripts": ["“What worked? What would you change next time?”"],
            "avoid": "Skipping reflection once the crisis passes.",
        },
    ]

    return moves, phase_cards[int(phase)]

def render_ipdp_phase_safe(phase_num, name, role, p_comm, s_comm, p_mot, s_mot):
    with st.expander(f"Phase {phase_num}", expanded=(phase_num == 1)):
        try:
            moves, phase_card = build_coaching_matrix_safe(
            ...
            with st.container(border=True):
                ...
                with c1:
                    st.markdown("**Aim**")
                    st.write(phase_card["aim"])
                with c2:
                    st.markdown("**Supervisor Role**")
                    st.write(phase_card["supervisor_role"])
                with c3:
                    st.markdown("**Common Pitfall**")
                    st.write(phase_card["common_pitfall"])

            with st.expander("🧭 Coaching Matrix (6 Core Moves)", expanded=True):
...
# Render all phases independently (crash-proof)
render_ipdp_phase_safe(1, name, role, p_comm, s_comm, p_mot, s_mot)
render_ipdp_phase_safe(2, name, role, p_comm, s_comm, p_mot, s_mot)
render_ipdp_phase_safe(3, name, role, p_comm, s_comm, p_mot, s_mot)
