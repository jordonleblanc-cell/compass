...
def display_guide(name, role, p_comm, s_comm, p_mot, s_mot):
    ...
    # --- 7-8: THRIVING/STRUGGLING ---
        ...
        # --- Phase selection (replaces the prior "Set as saved current phase" buttons) ---
        st.caption("Select the phase you are coaching in right now. This only changes which expander opens by default and which PDF you download.")
        selected_phase = st.selectbox(
            "Current IPDP phase",
            options=[1, 2, 3],
            index=(current_phase_num - 1),
            key=f"ipdp_phase_select__{name}"
        )
        st.session_state[phase_state_key] = int(selected_phase)
        current_phase_num = int(selected_phase)

        # --- Pedagogical "Why this phase matters" guidance (blue info boxes under each phase header) ---
        phase_why_matters = {
            1: (
                "Why this phase matters: Phase 1 is about building reliable fundamentals before you push for speed or complexity. "
                "Adults learn best when expectations are concrete, feedback is immediate, and the nervous system feels safe enough to try-and-miss. "
                "When a supervisor teaches boundaries (like practicing a respectful 'no'), clear routines, and short reflection habits, staff gain self-respect, "
                "capacity management, and consistency. That reduces burnout, improves decision quality under stress, and protects youth through predictable care."
            ),
            2: (
                "Why this phase matters: Phase 2 is where skill becomes behavior. The goal is transfer: taking what was taught in calm moments and using it "
                "during real shifts. Practice-and-feedback cycles (brief, frequent, specific) strengthen judgment, reduce 'freeze' responses, and make the right "
                "move easier to repeat. This phase is also where supervisors prevent drift by coaching patterns, not just incidents."
            ),
            3: (
                "Why this phase matters: Phase 3 is about durable autonomy and leadership. When staff can self-correct, coach peers, and make sound decisions "
                "without constant prompts, the whole unit stabilizes. This is adult learning at the highest level: people demonstrate mastery by teaching, "
                "generalizing skills across scenarios, and owning outcomes. The payoff is resilience, succession readiness, and fewer repeat issues."
            ),
        }
        ...
        def _build_ipdp_summary_pdf(staff_name: str, staff_role: str, phase_num: int) -> bytes:
            ...
            pdf.set_font("Arial", "", 11)
            pdf.multi_cell(0, 6, _safe_pdf_text(phase_why_matters.get(phase_num, "")))
            pdf.ln(2)

            # Phase 1 only: weekly coaching guidance for the full timeframe (0-6 months)
            if int(phase_num) == 1:
                weekly = [
                    "Week 1: Set clear expectations and daily routines; establish a short end-of-shift reflection.",
                    "Week 2: Practice a respectful 'no' (boundaries); reinforce that boundaries protect capacity and quality.",
                    "Week 3: Tighten documentation basics (what, when, who); coach accuracy over speed.",
                    "Week 4: Skill drill - calm tone and pace during minor pushback; reinforce one phrase to reuse.",
                    "Week 5: Proximity and presence - coach positioning, safety awareness, and predictable engagement.",
                    "Week 6: Follow-through - give a 1-sentence directive + 1 example; inspect for completion.",
                    "Week 7: De-escalation baseline - identify early signs; coach 'pause, breathe, lower volume'.",
                    "Week 8: Boundaries with time - coach saying yes with conditions (for example: 'I can at 3pm'), not overpromising.",
                    "Week 9: Youth engagement routine - plan one structured activity; coach setup and transitions.",
                    "Week 10: Reinforce standards - define what 'good' looks like in 2 observable behaviors.",
                    "Week 11: Coaching loop - supervisor models, staff practices, supervisor gives 1 improvement point.",
                    "Week 12: Midpoint check - review patterns, celebrate one win, set one focus for the next 2 weeks.",
                    "Week 13: Communication clarity - coach concise updates (headline first, details second).",
                    "Week 14: Boundary practice - teach how to decline a request without apologizing or over-explaining.",
                    "Week 15: Safety and escalation - rehearse the escalation pathway and when to call for support.",
                    "Week 16: Consistency across shifts - coach handoff notes and the next-step plan.",
                    "Week 17: Emotional regulation - coach self-check (ask: 'What am I feeling? What does the youth need?'), then act.",
                    "Week 18: Feedback receptivity - teach 'thank you, clarify, try again' to normalize learning.",
                    "Week 19: Boundaries with peers - coach respectful redirection (for example: 'Let's keep focus on youth' / 'Let's document first').",
                    "Week 20: Skill drill - respond to escalation with 1 grounding statement + 1 clear option.",
                    "Week 21: Build independence - reduce prompts; require a brief self-review after key moments.",
                    "Week 22: Reliability check - coach punctuality, task completion, and consistency in routine items.",
                    "Week 23: Prepare for Phase 2 - identify one skill to generalize under pressure; set a practice plan.",
                    "Week 24: Phase close-out - review growth, update goals, and confirm next phase focus and supports.",
                ]

                pdf.set_font("Arial", "B", 12)
                pdf.cell(0, 8, _safe_pdf_text("Weekly Coaching Guidance (Phase 1: 0-6 months)"), ln=True)
                pdf.set_font("Arial", "", 10)
                for w in weekly:
                    pdf.multi_cell(0, 5, _safe_pdf_text(f"- {w}"))
                pdf.ln(2)
        ...
        for phase_num in (1, 2, 3):
            ...
            with st.expander(exp_label, expanded=(phase_num == current_phase_num)):
                ...
                # Why this phase matters (pedagogical benefit for supervisors)
                st.info(phase_why_matters.get(phase_num, ""))

                # Phase coaching focus (from the assessment's IPDP phase text)
                ...
                with st.container(border=True):
                    ...
                    with colA:
                        ...
                        st.markdown(
                            """- Tighten expectations into **1 sentence + 1 example** (what to do, what it looks like)
    - Reduce ambiguity by naming **the standard** and **the boundary** (what is in-scope vs out-of-scope)
    - Make "good" visible: identify **2 observable behaviors** you will look for this week

**Supervisor prompts**
- "What does 'good' look like on this shift in two behaviors?"
- "What would I see or hear if you were doing this well?"

**What you are teaching (pedagogy)**
- Clear targets lower cognitive load, increase confidence, and speed up learning transfer under stress."""
                        )
                    with colB:
                        st.markdown("### Skill Application")
                        st.markdown(
                            """- Practice the skill in **real situations**, not just in theory (1-2 reps per shift)
    - Use a **3-step micro-loop**: *name the goal -> practice once -> give one adjustment*
    - Reinforce patterns: praise the specific behavior you want repeated, not personality

**Supervisor prompts**
- "Show me how you would say that in one calm sentence."
- "What will you do the same next time, and what will you adjust?"

**What you are teaching (pedagogy)**
- Frequent, specific feedback strengthens the neural pathway for the desired response and makes it easier to access during high arousal moments."""
                        )
                    with colC:
                        st.markdown("### Autonomy & Judgment")
                        st.markdown(
                            """- Increase decision reps: give **small choices** with clear guardrails
    - Coach judgment, not just rules: ask what data they noticed and what options they considered
    - Build self-correction: have them do a **30-second after-action** before you add your feedback

**Supervisor prompts**
- "What did you notice that made you choose that response?"
- "What is one risk, and what is one protective factor, in this moment?"

**What you are teaching (pedagogy)**
- Reflection builds metacognition. Metacognition is what turns a staff member from compliant to capable."""
                        )

                    with st.expander("How to use this matrix in a weekly coaching cadence"):
                        st.markdown(
                            """Use a consistent 10-12 minute structure so coaching stays predictable and does not feel punitive.

1) **Start with evidence (2 min):** one win and one specific moment to improve (facts, not impressions).
2) **Pick one lever (5 min):** choose *Structure*, *Skill*, or *Autonomy* based on what will make the biggest difference this week.
3) **Do one rep (2 min):** role-play a phrase, practice a directive, or rehearse a response to a common scenario.
4) **Set the next check (1 min):** name the one thing you will be looking for and when you will follow up.

**Phase emphasis tip**
- Phase 1: default to Structure (clear expectations, boundaries, routines)
- Phase 2: default to Skill Application (practice under real conditions)
- Phase 3: default to Autonomy (decision-making and self-correction)
"""
                        )
