...
# --- QUERY PARAM ACTIONS (for sticky CTA) ---
def _get_qp_value(key: str):
    """Return a single query param value (Streamlit may return str or list)."""
    try:
        # Newer Streamlit
...
def _clear_qp(*keys: str):
    try:
        # Newer Streamlit supports mutation
        for k in keys:
        ...
        for k in keys:
            qp.pop(k, None)
        st.experimental_set_query_params(**{k: qp[k][0] for k in qp if qp.get(k)})

def render_sticky_generate_footer(title: str, action: str = "generate_guide"):
    """Renders a fixed bottom CTA. Uses query params to trigger generation on rerun."""
    components.html(
        f"""
        <div class="sticky-footer">
          <div class="sticky-inner">
            <div class="sticky-title">{html.escape(title)}</div>
            <button class="sticky-btn" onclick="
...
st.markdown("""
    <style>
        ...
        /* --- Supervisor Dashboard Redesign Add-ons --- */

        /* Profile panel (Database selector) */
        .profile-panel {
            display: flex;
            gap: 16px;
            align-items: center;
        ...
        .profile-chips { display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; }
        .chip {
            padding: 6px 10px;
            border-radius: 999px;
            border: 1px solid var(--border-color);
        ...
        @media (prefers-color-scheme: dark) {
            .chip { background: rgba(138,180,248,0.12); }
        }
        ...
        @media (prefers-color-scheme: dark) {
            .sticky-footer { background: rgba(19,19,20,0.92); }
        }
        .sticky-inner {
            max-width: 1200px;
            margin: 0 auto;
        ...
        .sticky-title { color: var(--text-sub); font-size: 0.92rem; }
        .sticky-btn {
            appearance: none;
            border: none;
            border-radius: 12px;
            ...
            color: white;
            transition: transform 0.08s ease-in-out, background 0.2s;
        }
        .sticky-btn:hover { background: var(--primary-hover); transform: translateY(-1px); }
        .sticky-btn:active { transform: translateY(0px); }

        /* Chat bubble styling for scripts */
        .chat-bubble {
            max-width: 900px;
            padding: 12px 14px;
            border-radius: 16px;
        ...
        @media (prefers-color-scheme: dark) {
            .chat-bubble { background: rgba(138,180,248,0.12); }
        }
        ...
        /* Make Streamlit tabs look more "pill" and distinct */
        ...
        button[role="tab"][aria-selected="true"] {
            ...
            color: var(--primary) !important;
            box-shadow: 0 2px 10px rgba(26,115,232,0.12) !important;
        }
    ...
    with sub1:
        if not df.empty:
            ...
            if sel:
                ...
                # Profile panel (same data, modern layout)
                initials = "".join([p[0].upper() for p in str(d.get("name","")).split()[:2] if p]) or "?"
                subtitle_bits = []
                if d.get("role"): subtitle_bits.append(str(d.get("role")))
                if d.get("cottage"): subtitle_bits.append(str(d.get("cottage")))
                ...
                st.markdown(
                    ...
                    <div class="profile-panel">
                        ...
                        <div class="profile-meta">
                            <div class="profile-name">{html.escape(str(d.get("name","")))}</div>
                            <div class="profile-sub">{html.escape(subtitle)}</div>
                        </div>
                        <div class="profile-chips">
                            <span class="chip">Style: {html.escape(str(d.get('p_comm','')))}</span>
                            <span class="chip">Drive: {html.escape(str(d.get('p_mot','')))}</span>
                            <span class="chip">Role: {html.escape(str(d.get('role','')))}</span>
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True
                ...
                # Trigger generation via sticky footer (primary CTA)
                render_sticky_generate_footer(f"Selected: {d.get('name','')}")

                # Also support a non-JS fallback button (kept, but not the primary CTA)
                fallback_click = st.button("Generate Guide", type="primary", width="stretch", key="t1_generate_fallback")

                qp_action = _get_qp_value("action")
                should_generate = (qp_action == "generate_guide") or fallback_click

                if should_generate:
                    # Prevent repeated generation on rerun
                    if qp_action == "generate_guide":
                        _clear_qp("action")
                    ...
                    with t_prime:
                        ...
                        with c_b:
                            ...
                            for i, (cat, text) in enumerate(clash_p['scripts'].items()):
                                with script_tabs[i]:
                                    st.markdown(f'<div class="chat-bubble"><span class="bubble-label">{html.escape(cat)}</span><div>{html.escape(text)}</div></div>', unsafe_allow_html=True)
...
# 4. CAREER PATHFINDER
    ...
    if not df.empty:
        ...
        if cand and role:
            ...
            if path:
                # Tabs (same sections, cleaner layout)
                tab_labels = ["🔄 Understanding the Shift", "🧠 Psychological Block", "🗣️ The Conversation", "✅ Assignment"]
                has_debrief = isinstance(path.get("debrief_questions"), (list, tuple)) and len(path.get("debrief_questions")) > 0
                if has_debrief:
                    tab_labels.append("🧠 Debrief Questions")
                t = st.tabs(tab_labels)
                ...
                with t[0]:
                    ...
                    if isinstance(shift, dict):
                        ...
                        # Keep compatibility if old schema
                        st.write(path.get("shift", ""))
                ...
                # --- TAB 5: DEBRIEF (optional) ---
                if has_debrief:
                    with t[4]:
                        with st.container(border=True):
                            st.markdown("### 🧠 Post-Assignment Debrief Questions")
                            for q in path['debrief_questions']:
                                st.markdown(f"- {q}")
